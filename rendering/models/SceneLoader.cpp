...
std::vector<std::shared_ptr<GraphicsModelBase>> SceneLoader::getNextModel() {
  ...
  std::shared_ptr<GraphicsModel<ge::gl::Buffer>> result =
      std::dynamic_pointer_cast<GraphicsModel<ge::gl::Buffer>>(
          modelLoader.loadModel(model->FindAttribute("filename")->Value(),
                                model->FindAttribute("name")->Value()));
  if (strncmp(model->Value(), "model", 5) == 0) {
    auto posEl = model->FirstChildElement("position");
    glm::vec3 position{posEl->FloatAttribute("x", 0),
                       posEl->FloatAttribute("y", 0),
                       posEl->FloatAttribute("z", 0)};
    auto rotEl = model->FirstChildElement("rotation");
    glm::vec3 rotation{rotEl->FloatAttribute("x", 0),
    ...
    glm::vec3 genStart{genPosStartEl->FloatAttribute("x", 0),
                       genPosStartEl->FloatAttribute("y", 0),
                       genPosStartEl->FloatAttribute("z", 0)};
    auto genPosStepEl = model->FirstChildElement("genPosStep");
    glm::vec3 genStep{genPosStepEl->FloatAttribute("x", 0),
                      genPosStepEl->FloatAttribute("y", 0),
    ...
    const auto genEnd =
        genStart +
        genStep * (std::pow<float>(instanceCount, 1.f / genAxisCount));
    for (auto [x, y, z] : range<float, 3>({genStart.x, genStart.y, genStart.z},
                                          {genEnd.x, genEnd.y, genEnd.z},
...
SceneLoader::iterator::iterator(SceneLoader &loader, bool isEnd)
    : loader(&loader) {
  if (!isEnd)
