...
freetypeglxx::TextureFont *Font::getData(float size) {
  if (auto iter = std::find_if(fontSizes.begin(), fontSizes.end(), [size](const auto &val) { return val.first == size; });
      iter != fontSizes.end()) {
...
Font::Font(Font &&other) noexcept : fontSizes(std::move(other.fontSizes)), fontManager(other.fontManager) {}
