...
std::array<glm::vec3, 4> sdlRectToGLCoordinates(const SDL_Rect &rect,
  ...
  float xLeft =
      static_cast<float>(rect.x) / static_cast<float>(screenWidth) * 2;
  float xRight =
      static_cast<float>(rect.x + rect.w) / static_cast<float>(screenWidth) * 2;
  float yTop =
      static_cast<float>(rect.y) / static_cast<float>(screenHeight) * 2;
  float yBottom = static_cast<float>(rect.y + rect.h) /
