...
SparseBuffer::SparseBuffer(GLsizeiptr size, const void *data) : Buffer(size, data, GL_SPARSE_STORAGE_BIT_ARB) {
  getContext().glGetIntegerv(GL_SPARSE_BUFFER_PAGE_SIZE_ARB, &sparsePageSize);
}

SparseBuffer::SparseBuffer(const ge::gl::FunctionTablePointer &table) : Buffer(table) {
  getContext().glGetIntegerv(GL_SPARSE_BUFFER_PAGE_SIZE_ARB, &sparsePageSize);
}

SparseBuffer::SparseBuffer(const ge::gl::FunctionTablePointer &table, GLsizeiptr size, const void *data)
    : Buffer(table, size, data, GL_SPARSE_STORAGE_BIT_ARB) {
  getContext().glGetIntegerv(GL_SPARSE_BUFFER_PAGE_SIZE_ARB, &sparsePageSize);
}

template <typename T> SparseBuffer::SparseBuffer(const std::vector<T> &data) : Buffer(data, GL_SPARSE_STORAGE_BIT_ARB) {
  getContext().glGetIntegerv(GL_SPARSE_BUFFER_PAGE_SIZE_ARB, &sparsePageSize);
}

template <typename T> SparseBuffer::SparseBuffer(const ge::gl::FunctionTablePointer &table, const std::vector<T> &data)
