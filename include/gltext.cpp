...
GLT_API GLTtext *gltCreateText(void) {
  ...
  ge::gl::glGenVertexArrays(1, &text->_vao);
  ge::gl::glGenBuffers(1, &text->_vbo);
  ...
  ge::gl::glBindVertexArray(text->_vao);

  ge::gl::glBindBuffer(GL_ARRAY_BUFFER, text->_vbo);

  ge::gl::glEnableVertexAttribArray(_GLT_TEXT2D_POSITION_LOCATION);
  ge::gl::glVertexAttribPointer(
  ...
  ge::gl::glEnableVertexAttribArray(_GLT_TEXT2D_TEXCOORD_LOCATION);
  ge::gl::glVertexAttribPointer(
...
GLT_API void gltDeleteText(GLTtext *text) {
  ...
  if (text->_vao) {
    ge::gl::glDeleteVertexArrays(1, &text->_vao);
    text->_vao = GLT_NULL_HANDLE;
  }

  if (text->_vbo) {
    ge::gl::glDeleteBuffers(1, &text->_vbo);
...
GLT_API void gltBeginDraw() {
  ge::gl::glUseProgram(_gltText2DShader);

  ge::gl::glActiveTexture(GL_TEXTURE0);
  ge::gl::glBindTexture(GL_TEXTURE_2D, _gltText2DFontTexture);
}

GLT_API void gltEndDraw() {}

#define _gltDrawText() \
ge::gl::glUniformMatrix4fv(_gltText2DShaderMVPUniformLocation, 1, GL_FALSE, mvp); \
	\
ge::gl::glBindVertexArray(text->_vao); \
ge::gl::glDrawArrays(GL_TRIANGLES, 0, text->vertexCount);
...
GLT_API void gltColor(GLfloat r, GLfloat g, GLfloat b, GLfloat a) {
  ge::gl::glUniform4f(_gltText2DShaderColorUniformLocation, r, g, b, a);
}

GLT_API void gltGetColor(GLfloat *r, GLfloat *g, GLfloat *b, GLfloat *a) {
  GLfloat color[4];
  ge::gl::glGetUniformfv(_gltText2DShader, _gltText2DShaderColorUniformLocation,
                         color);
...
GLT_API void _gltGetViewportSize(GLint *width, GLint *height) {
  GLint dimensions[4];
  ge::gl::glGetIntegerv(GL_VIEWPORT, dimensions);
...
GLT_API void _gltUpdateBuffers(GLTtext *text) {
  ...
  ge::gl::glBindBuffer(GL_ARRAY_BUFFER, text->_vbo);
  ge::gl::glBufferData(GL_ARRAY_BUFFER,
...
GLT_API void gltTerminate(void) {
  if (_gltText2DShader != GLT_NULL_HANDLE) {
    ge::gl::glDeleteProgram(_gltText2DShader);
    _gltText2DShader = GLT_NULL_HANDLE;
  }

  if (_gltText2DFontTexture != GLT_NULL_HANDLE) {
    ge::gl::glDeleteTextures(1, &_gltText2DFontTexture);
...
GLT_API GLboolean _gltCreateText2DShader(void) {
  ...
  vertexShader = ge::gl::glCreateShader(GL_VERTEX_SHADER);
  ge::gl::glShaderSource(vertexShader, 1, &_gltText2DVertexShaderSource, NULL);
  ge::gl::glCompileShader(vertexShader);

  ge::gl::glGetShaderiv(vertexShader, GL_COMPILE_STATUS, &compileStatus);

  if (compileStatus != GL_TRUE) {
#ifdef GLT_DEBUG_PRINT
    ge::gl::glGetShaderiv(vertexShader, GL_INFO_LOG_LENGTH, &infoLogLength);
    ...
    if (infoLogLength > 1) {
      ...
      ge::gl::glGetShaderInfoLog(vertexShader, infoLogSize, NULL, infoLog);
    ...
    ge::gl::glDeleteShader(vertexShader);
    gltTerminate();
  ...
  fragmentShader = ge::gl::glCreateShader(GL_FRAGMENT_SHADER);
  ge::gl::glShaderSource(fragmentShader, 1, &_gltText2DFragmentShaderSource,
                         NULL);
  ge::gl::glCompileShader(fragmentShader);

  ge::gl::glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, &compileStatus);

  if (compileStatus != GL_TRUE) {
#ifdef GLT_DEBUG_PRINT
    ge::gl::glGetShaderiv(fragmentShader, GL_INFO_LOG_LENGTH, &infoLogLength);
    ...
    if (infoLogLength > 1) {
      ...
      ge::gl::glGetShaderInfoLog(fragmentShader, infoLogSize, NULL, infoLog);
    ...
    ge::gl::glDeleteShader(vertexShader);
    ge::gl::glDeleteShader(fragmentShader);
    gltTerminate();
  ...
  _gltText2DShader = ge::gl::glCreateProgram();

  ge::gl::glAttachShader(_gltText2DShader, vertexShader);
  ge::gl::glAttachShader(_gltText2DShader, fragmentShader);

  ge::gl::glBindAttribLocation(_gltText2DShader, _GLT_TEXT2D_POSITION_LOCATION,
                               "position");
  ge::gl::glBindAttribLocation(_gltText2DShader, _GLT_TEXT2D_TEXCOORD_LOCATION,
                               "texCoord");

  ge::gl::glBindFragDataLocation(_gltText2DShader, 0, "fragColor");

  ge::gl::glLinkProgram(_gltText2DShader);

  ge::gl::glDetachShader(_gltText2DShader, vertexShader);
  ge::gl::glDeleteShader(vertexShader);

  ge::gl::glDetachShader(_gltText2DShader, fragmentShader);
  ge::gl::glDeleteShader(fragmentShader);

  ge::gl::glGetProgramiv(_gltText2DShader, GL_LINK_STATUS, &linkStatus);

  if (linkStatus != GL_TRUE) {
#ifdef GLT_DEBUG_PRINT
    ge::gl::glGetProgramiv(_gltText2DShader, GL_INFO_LOG_LENGTH,
                           &infoLogLength);
    ...
    if (infoLogLength > 1) {
      ...
      ge::gl::glGetProgramInfoLog(_gltText2DShader, infoLogSize, NULL, infoLog);
  ...
  _gltText2DShaderMVPUniformLocation =
      ge::gl::glGetUniformLocation(_gltText2DShader, "mvp");
  _gltText2DShaderColorUniformLocation =
      ge::gl::glGetUniformLocation(_gltText2DShader, "color");

  ge::gl::glUniform1i(ge::gl::glGetUniformLocation(_gltText2DShader, "diffuse"),
                      0);
...
#undef _GLT_TEX_SET_PIXEL
  ...
  ge::gl::glGenTextures(1, &_gltText2DFontTexture);
  ge::gl::glBindTexture(GL_TEXTURE_2D, _gltText2DFontTexture);

  ge::gl::glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texWidth, texHeight, 0,
