...
template <typename T, typename U> struct OneWayBinding {
  ...
  OneWayBinding(observable::value<T> &a, observable::value<U> &b,
                F1 transformA = TransformAtoBfnc{[](const U &val) {
                  return T(val);
                }})
...
template <typename T, typename U> struct OneWayBindingVal {
  ...
  using UnsubscriberB = decltype(std::declval<observable::value<U>>().subscribe(
      std::declval<std::function<void(const U &val)>>()));
  template <typename F1 = TransformAtoBfnc>
  OneWayBindingVal(T &a, observable::value<T> &b,
                F1 transformA = TransformAtoBfnc{[](const T &val) -> U {
                  return U(val);
                }})
      : a(a), b(b), tA(transformA) {
    unsubscriberB = b.subscribe([this](const U& val) {
      this->a = val;
    });
  }
