...
void ModelRenderer::addModel(std::shared_ptr<GraphicsModelBase> model) {
  models.emplace_back(std::move(model));
}
void ModelRenderer::render(const std::shared_ptr<ge::gl::Program> &program, glm::mat4 view, bool wa) {
  for (auto &model : models) {
    if (wa) {
      auto modelView = view * model->modelMatrix.getRef();
      program->setMatrix4fv("modelView", &modelView[0][0]);
    }
