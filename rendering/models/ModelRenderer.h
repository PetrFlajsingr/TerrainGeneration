...
public:
  ...
  std::shared_ptr<GraphicsModelBase> loadModel(std::string name, const std::string &id);
...
public:
  ...
  std::optional<std::shared_ptr<GraphicsModelBase>> modelById(const GraphicsModelBase::Id &id);
...
std::shared_ptr<GraphicsModelBase> ObjModelLoader::loadModel(std::string name, const std::string &id) {
  if (name.substr(name.size() - 4) == ".obj") {
    name = name.substr(0, name.size() - 4);
  }
  using namespace MakeRange;
  const auto path = assetsPath + '/' + name + ".obj";
  auto result = std::make_shared<GraphicsModel<BufferType>>(id, GraphicsModelBase::Type::UniformColor);
  ...
  for (auto i : range(0, attribs.vertices.size(), 3)) {
    vertices.emplace_back(attribs.vertices[i], attribs.vertices[i + 1], attribs.vertices[i + 2], 1);
  }
  ...
  for (auto index : shapes[0].mesh.indices) {
    ...
    normals[index.vertex_index] += glm::vec3{attribs.normals[index.normal_index * 3], attribs.normals[index.normal_index * 3 + 1],
  ...
  result->vertexArray->addAttrib(result->vertexBuffer, 0, 4, GL_FLOAT, sizeof(float) * 4, 0, GL_FALSE);
  result->vertexArray->addAttrib(result->normalBuffer, 1, 3, GL_FLOAT, sizeof(float) * 3, 0, GL_FALSE);
