...
std::array<glm::vec3, 4> sdlRectToGLCoordinates(const SDL_Rect &rect,
  ...
  float xLeft = static_cast<float>(rect.x) / screenWidth * 2;
  float xRight = static_cast<float>(rect.x + rect.w) / screenWidth * 2;
  float yTop = static_cast<float>(rect.y) / screenHeight * 2;
  float yBottom = static_cast<float>(rect.y + rect.h) / screenHeight * 2;
