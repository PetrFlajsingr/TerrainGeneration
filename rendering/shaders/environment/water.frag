...
float waterNoise(vec3 Position) {
    return
    + simplexNoise(Position * 3) /3
    + simplexNoise(Position*2)/2
    + simplexNoise(Position/10)
    ;
}
...
vec3 calcNormal(vec3 Position) {
    const float d = 1;
    vec3 grad;
    grad.x = waterNoise(Position.xyz + vec3(d, 0, 0)) - waterNoise(Position.xyz + vec3(-d, 0, 0));
    grad.y = waterNoise(Position.xyz + vec3(0, d, 0)) - waterNoise(Position.xyz + vec3(0, -d, 0));
    grad.z = waterNoise(Position.xyz + vec3(0, 0, d)) - waterNoise(Position.xyz + vec3(0, 0, -d));
...
void main() {
    ...
    vec3 lighting = (ambient + (diffuse + specular)) * objColor;

    //color = vec4(0, 0, 1, 1) + surfaceValue + vec4(c, 0);
    color = vec4(lighting, 0.6);
}