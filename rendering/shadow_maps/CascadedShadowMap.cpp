...
CascadedShadowMap::CascadedShadowMap(unsigned int cascadeCount, unsigned int size)
    : cascadeCount(cascadeCount), size(size),
      program(std::make_shared<ge::gl::Program>("shadow_map/sm"_vert, "shadow_map/sm"_frag)) {
  ...
  ge::gl::glBindTexture(GL_TEXTURE_2D_ARRAY, depthMap);
  ge::gl::glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_DEPTH_COMPONENT32F, size, size, cascadeCount, 0, GL_DEPTH_COMPONENT, GL_FLOAT,
                       nullptr);
...
void CascadedShadowMap::bindRender(const std::shared_ptr<ge::gl::Program> &program) {
  ...
  for (auto i : range(cascadeCount)) {
    ge::gl::glUniformMatrix4fv(
        ge::gl::glGetUniformLocation(program->getId(), ("lightViewProjectionMatrices[" + std::to_string(i) + "]").c_str()), 1,
        GL_FALSE, &cascadedMatrices[i][0][0]);
  }
...
void CascadedShadowMap::calculateOrthoMatrices(const glm::mat4 &cameraProjection, const glm::mat4 &cameraView, float cameraNear,
  ...
  for (unsigned int cascadeIterator = 0; cascadeIterator < cascadeCount; ++cascadeIterator) {
    const float prevSplitDistance = cascadeIterator == 0 ? minDistance : cascadeSplits[cascadeIterator - 1];
    ...
    const glm::vec3 lightDirection = frustumCenter - glm::normalize(lightDir) * -minExtents.z;
    lightViewMatrix[cascadeIterator] = glm::mat4(1.0f);
    lightViewMatrix[cascadeIterator] = glm::lookAt(lightDirection, frustumCenter, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    const auto shadowMatrix = lightOrthoMatrix[cascadeIterator] * lightViewMatrix[cascadeIterator];
    ...
    cascadeSplitArray[cascadeIterator] = (cameraNear + splitDistance * clipDist) * -1.0f;
...
void CascadedShadowMap::setLightDir(const glm::vec3 &lightDir) { CascadedShadowMap::lightDir = lightDir; }

const std::vector<float> &CascadedShadowMap::getCascadeSplits() const { return cascadeSplitArray; }
...
void CascadedShadowMap::setLambda(float lambda) { CascadedShadowMap::lambda = lambda; }

float CascadedShadowMap::getMinDistance() const { return minDistance; }
void CascadedShadowMap::setMinDistance(float minDistance) { CascadedShadowMap::minDistance = minDistance; }

float CascadedShadowMap::getMaxDistance() const { return maxDistance; }
void CascadedShadowMap::setMaxDistance(float maxDistance) { CascadedShadowMap::maxDistance = maxDistance; }
